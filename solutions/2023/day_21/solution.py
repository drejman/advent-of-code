...
class Solution(StrSplitSolution):
    ...
    def _is_inside_grid(self, coordinates: Coordinates):
        return (
            0 <= coordinates.col <= self._max_size[0]
            and 0 <= coordinates.row <= self._max_size[1]
        )
    ...
    def _get_map_coordinates(self, coordinates: Coordinates) -> Coordinates:
        if self._is_inside_grid(coordinates):
            return coordinates
        return Coordinates(
            col=(coordinates.col % (self._max_size.col + 1)),
            row=(coordinates.row % (self._max_size.row + 1)),
        )
